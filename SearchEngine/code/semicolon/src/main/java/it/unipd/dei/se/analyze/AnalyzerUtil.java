...
/**
 * Helper class to load stop lists and <a href="http://opennlp.apache.org/" target="_blank">Apache
 * OpenNLP</a> models from the {@code resource} directory as well as consume {@link TokenStream}s
...
public class AnalyzerUtil {
    ...
    /**
     * Consumes a {@link TokenStream} for the given text by using the provided {@link Analyzer} and
     * prints diagnostic information about all the generated tokens and their {@link
     * org.apache.lucene.util.Attribute}s.
     *
    ...
    public static void consumeTokenStream(final Analyzer a, final String t) throws IOException {
        ...
        // Whether the token is a keyword. Keyword-aware TokenStreams/-Filters skip modification of
        ...
        final PositionIncrementAttribute tokenPositionIncrement =
                stream.addAttribute(PositionIncrementAttribute.class);

        // The number of positions occupied by a token
        final PositionLengthAttribute tokenPositionLength =
        ...
        // Some printf to look some debug
        ...
        try {
            ...
            while (stream.incrementToken()) {
                ...
                System.out.printf(
                        "  - position increment: %d%n", tokenPositionIncrement.getPositionIncrement());
                System.out.printf("  - position length: %d%n", tokenPositionLength.getPositionLength());
                System.out.printf(
                        "  - offset: [%d, %d]%n", tokenOffset.startOffset(), tokenOffset.endOffset());
    ...
    public static CharArraySet loadStopList(final String stopFile) {
        ...
        try (BufferedReader in = new BufferedReader(
                new InputStreamReader(CL.getResourceAsStream(stopFile)))){

            // Get a reader for the file containing the stop list

            // Read the stop list
            return StopFilter.makeStopSet(in.lines().toList(), true);
    ...
    /**
     * Loads the required Apache OpenNLP POS tagger model among those available in the {@code
     * resources} folder.
     *
    ...
    static NLPPOSTaggerOp loadPosTaggerModel(final String modelFile) {
        ...
        try {
            ...
            throw new IllegalStateException(
                    String.format("Unable to load the model %s: %s", modelFile, e.getMessage()), e);
        }
    ...
    /**
     * Loads the required Apache OpenNLP sentence detector model among those available in the {@code
     * resources} folder.
     *
     * @param modelFile the name of the file containing the model.
     * @return the required Apache OpenNLP model.
    ...
    /**
     * Loads the required Apache OpenNLP tokenizer model among those available in the {@code
     * resources} folder.
     *
     * @param modelFile the name of the file containing the model.
     * @return the required Apache OpenNLP model.
    ...
    /**
     * Loads the required Apache OpenNLP lemmatizer model among those available in the {@code
     * resources} folder.
     *
     * @param modelFile the name of the file containing the model.
     * @return the required Apache OpenNLP model.
    ...
    /**
     * Loads the required Apache OpenNLP NER tagger model among those available in the {@code
     * resources} folder.
     *
     * @param modelFile the name of the file containing the model.
     * @return the required Apache OpenNLP model.
    ...
    public static SynonymMap buildSynonymMap(String synonymFile) {
        SynonymMap.Builder smb = new SynonymMap.Builder();
        boolean keepOrig = false;
        try {
            Reader in = new BufferedReader(new InputStreamReader(CL.getResourceAsStream(synonymFile)));
            BufferedReader file = new BufferedReader(in);
            String line;
            while ((line = file.readLine()) != null) {
                String[] terms = line.split(",");
                for (String term : terms)
                    for (String termRef : terms) smb.add(new CharsRef(term), new CharsRef(termRef), keepOrig);
            }
            file.close();
            in.close();
            return smb.build();
