...
/**
 * Implements the Lovins stemming algorithm. A collection of implementations of the Lovins stemmer
 * is described in &quot;Development of a Stemming Algorithm&quot; by Julie Beth Lovins, published
 * in <i>Mechanical translation and computational linguistics</i>, 11:22-31, 1968.
 *
 * <p>Code adapted from: <a href="http://www.cs.waikato.ac.nz/~eibe/stemmers/"
 * target="_blank">http://www.cs.waikato.ac.nz/~eibe/stemmers/</a>
 *
 * @author Nicola Ferro
 * @version 1.00
 * @since 1.00
...
public class LovinsStemFilter extends TokenFilter {
    ...
    /**
     * Enters C version compatibility mode if set to true (emulates features of the original C
     * implementation that are inconsistent with the algorithm as described in Lovins's paper)
     */
            ...
            if (length - el > 1) {
                ...
                if (conditionCode != null) {
                    switch (conditionCode) {
                        ...
                        case "N":
                            if ((length - el > 3)
                                    || ((length - el == 3) && ((word.charAt(length - el - 3) != 's')))) {
                                return word.substring(0, length - el);
                            }
                            break;
                        case "O":
    ...
    private static final String recodeEnding(String word) {
        ...
        if (word.endsWith("her")) {
            if ((lastPos - 3 < 0)
                    || ((word.charAt(lastPos - 3) != 'p') && (word.charAt(lastPos - 3) != 't'))) {
